...
function makeNodeReduceIterator(res: any, key: string) {
  ...
  return async (allResult: undefined | string[], node: Node) => {
    ...
    try {
      ...
      let gen = new Generator(key, node, nodeUrl, nodeSource, genDir)
      let str = await gen.exec(res.markdown, res.promise)
