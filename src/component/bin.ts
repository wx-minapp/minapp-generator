...
import WxParser from './platform/wx/WxParser'
...
cli({
  ...
  'f | force': '<boolean> 不使用缓存，每次都去官方重新拉数据（索引页面的缓存不受此影响）'
}).parse(function(res) {
  ...
  series(platforms, async (p) => {
    ...
    const Parser = require(parserFile).default as (typeof WxParser)

    const parser = new Parser({
      includePageNames: res._,
      ignorePageCache: res.force
    })
